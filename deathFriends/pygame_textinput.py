...
class TextInput:
    def __init__(
            ...
            text_color=WHITE,
            cursor_color=WHITE,
            repeat_keys_initial_ms=20,
            repeat_keys_interval_ms=20,
            max_string_length=-1,
            password=False):
        ...
        if not os.path.isfile(font_family):
            font_family = pg.font.match_font(font_family)

        self.font_object = pg.font.Font(font_family, font_size)

        # Text-surface will be created during the first update call:
        self.surface = pg.Surface((25, 25))
        ...
        for event in events:
            if event.type == pg.KEYDOWN:
                ...
                if event.key not in self.keyrepeat_counters:
                    if not event.key == pg.K_RETURN: # Filters out return key, others can be added as necessary
